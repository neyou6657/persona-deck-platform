...
2. 点击“保存并重启 Agent”后，relay 会给该 worker 下发 `control/restart` 消息，消息里包含
   `enabledSkills`。
3. HF Space worker 收到重启配置后，会执行技能同步（从 `SKILLS_REPO_URL` + `SKILLS_REPO_SUBDIR`
   拉取）。
4. worker 用下发的 `enabledSkills` 过滤仓库中的技能目录，只保留命中的技能到
