...
import me.rerere.rikkahub.data.relay.RemoteRelayApi
...
val dataSourceModule = module {
    ...
    single<RelayApi> {
        RemoteRelayApi(
            client = get(),
            configProvider = { get<RelayConfigStore>().load() }
        )
    }

    single {
        RelayPersonaRepository(api = get())
    }

    single {
        RelayConversationRepository(api = get())
    }

    single {
        RelayPollingChatRepository(api = get())
    }
}
