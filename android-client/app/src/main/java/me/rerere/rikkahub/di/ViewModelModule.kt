...
import me.rerere.rikkahub.ui.pages.relay.RelayChatVM
import me.rerere.rikkahub.ui.pages.relay.RelayConversationListVM
...
val viewModelModule = module {
    ...
    viewModelOf(::StatsVM)
    viewModel<RelayConversationListVM> {
        RelayConversationListVM(
            personaId = it.get<String>(),
            personaName = it.get<String>(),
            relayConversationRepository = get(),
        )
    }
    viewModel<RelayChatVM> {
        RelayChatVM(
            personaId = it.get<String>(),
            personaName = it.get<String>(),
            initialConversationId = it.get<String>(),
            initialTitle = it.get<String>(),
            createNew = it.get<Boolean>(),
            relayConversationRepository = get(),
            relayChatRepository = get(),
        )
    }
}
